...
(function ($) {
  ...
  function pjax(options) {
    ...
    options.complete = function (xhr, textStatus) {
      // PJAX 요청이 완료된 후 실행되는 코드
      var $newContent = $(xhr.responseText);

      // 새로운 메타 디스크립션과 키워드를 가져옴
      var newDescription = $newContent
        .filter('meta[name="description"]')
        .attr("content");
      var newKeywords = $newContent
      ...
      // 추가로 메타 키워드도 업데이트할 수 있습니다.
      $('meta[name="keywords"]').attr("content", newKeywords);

      // OG 업데이트
      $('meta[property="og:keywords"]').attr("content", newKeywords);
      $('meta[property="og:title"]').attr("content", newKeywords);
      var currentUrl = window.location.href;
      $('meta[property="og:url"]').attr("content", currentUrl);

      // Google 광고 스크립트 다시 로드
  ...
  function extractContainer(data, xhr, options) {
    ...
    if (obj.contents) {
      ...
      obj.scripts.each(function () {
        $(this).after(
          '<div style="display: none" data-src="' +
            $(this).attr("src") +
            '"></div>'
        );
        $(this).remove();
